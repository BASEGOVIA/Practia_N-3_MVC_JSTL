...
<!DOCTYPE html><html>
    ...
    <table border="3">
        <tr>
            <th>Id</th> <th>Titulo</th><th>Autor</th><th>Disponible</th>
            <th>Categoria</th> <th>Editar</th> <th>Eliminar</th>
        </tr>
        <%if(libro!=null){for(Libros item: libro){%>
        <tr>
            <td><%= item.getId()%></td><td><%= item.getTitulo()%></td><td><%= item.getAutor()%></td>
            <td><%= item.getDisponible()%></td><td><%= item.getCate().getCategoria() %></td>
